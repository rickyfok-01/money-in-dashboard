...
- **Adding a tab:** tabs use strict sequential numbering 00–25; inserting requires renumbering the `TABS` tail. Write `docs/NN-*.md` spec first, then implement.
...
```
data.js → core.js → charts.js → tabs-summary.js → tabs-detail.js → tabs-settings.js → tabs-alloc.js → tabs-summary-v2.js → app.js
```
...
- **26 tabs** (0–25), each with a spec in `docs/NN-*.md`. `docs/00-architecture.md` covers design system + data model.
- **Global `DATA`** object: `DATA.rows` (bill data), `DATA.pym` (payment data), `DATA.names` (scheme/trustee name lookup), `DATA.aoAging` (AO aging), `DATA.ddi30`/`DATA.ddiAging` (DDI), `DATA.dda30`/`DATA.ddaAging` (DDA).
...
- `data/dda-*.csv`, `data/ddi-*.csv` → `DATA.dda30`/`DATA.ddaAging`, `DATA.ddi30`/`DATA.ddiAging`
- `data/con-pym-ao-aging-*.csv` → `DATA.aoAging`
