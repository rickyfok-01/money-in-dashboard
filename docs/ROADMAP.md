...
- **Tab patterns:** A=`renderGrouped` (generic dimension) · B=custom overview · C=pym-style matrix · D=`renderCross` (crosstab)
...
| 01 | Bill audit | Confirm Bill L1/L2/L3 coverage; gap-fill if any; refresh stale `data.js` + add `constant-scheme-info.xlsx` (pre-flight). | `rows` | — | — | 0–1 | next |
